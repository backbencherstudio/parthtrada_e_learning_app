...
final expertRiverpod = StateNotifierProvider<ExpertRiverpod, ExpertState>(
  (ref) => ExpertRiverpod(),
);
...
class ExpertRiverpod extends StateNotifier<ExpertState> {
  ...
  Future<void> fetchReviews() async {
    final reviewList =
        expertReviews
            .map((review) => ExpertReviewModel.fromJson(review))
            .toList();
    state = state.copyWith(expertReviewList: reviewList);
  }
  ...
  Future<void> addReviews({required ExpertReviewModel userReview}) async {
    ...
    state = state.copyWith(expertReviewList: state.expertReviewList);
  }
